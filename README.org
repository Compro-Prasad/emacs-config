...
* The tutorial
  ...
  tutorial accessible from the startup screen of Emacs. This will help you
  understand the terms used in Emacs.
* Copyright notice
...
* Enable debugging
  The following will enable stacktrace / backtrace in Emacs so that I can debug
  the solution right away. Disabling this (/i.e./ setting it to =nil=) will only
  show error in the =*Messages*= buffer with no hint on the exact function(s)
  failing.
...
* Aliases
  ...
  #+end_src
* Change load-path
  In Emacs 29 and above =user-lisp= is the default location for custom lisp files.
  =load-path= is a list that needs to be modified if you are planning to load
  Emacs Lisp files from other filepath locations in your system.
  #+begin_src emacs-lisp :tangle (early-init)
    (when (<= emacs-major-version 29)
      (add-to-list 'load-path (expand-file-name "user-lisp" emacs-d)))
  #+end_src
* Cache directory
  =cache-d= will be used numerous times in this config to move cache files from
  installed packages in this directory.
  #+begin_src emacs-lisp :tangle (early-init)
    (setq cache-d (locate-user-emacs-file (concat emacs-d ".cache/"))
          package-user-dir (concat cache-d "elpa/"))
  #+end_src
* TODO Connecting to client
  ...
  #+begin_src emacs-lisp :tangle (early-init)
    (use-package server
      ...
      (when (not (server-running-p))
        ...
        (server-start)))
  #+end_src
  A daemon has no initial frame.  Shortly after startup, create a graphical
  frame when an X11 or Wayland display is available.  The delay gives an
  =emacsclient --create-frame= invocation that started the daemon time to make
  its requested frame first, avoiding two frames.  A daemon started directly
  still gets a graphical frame, while a headless daemon needs =emacsclient -t=
  to attach a terminal.

  Themes loaded during daemon initialization may select terminal colors because
  no graphical frame exists yet.  Reload the enabled themes once on the first
  graphical frame so GUI clients use the intended color palette.
  #+begin_src emacs-lisp :tangle (early-init)
    ...
    (when (daemonp)
      (add-hook 'after-make-frame-functions
                #'compro/reload-themes-on-first-graphical-frame))
  #+end_src
  This config uses a local Unix socket (not TCP), so the relevant environment
  variable for =emacsclient= is =EMACS_SOCKET_NAME=, not =EMACS_SERVER_FILE=
  (which is only read for TCP connections). Make sure it is set like the
  following:
  #+begin_src sh
    export EMACS_SOCKET_NAME="$HOME/.config/emacs/.cache/server/server"
  #+end_src
...
* Check Operating System
  The following variables can be used to check and execute platform specific
  code:
  1. =is-windows=
  2. =is-unix=
  3. =is-gnu=
  ...
  5. =is-mac=
  6. =is-bsd=
  It isn't fully accurate so make changes as per your needs.
  #+begin_src emacs-lisp :tangle init.el
    (require 'seq)
    (setq is-windows
          (seq-find
...
* Tangle =README.org= to =init.el=
  The following code enables tangling / exporting code blocks from =README.org= to
  =init.el= when saving =README.org= from inside Emacs.
...
* Setup package repositories
  Setup MELPA and Non GNU repositories using =package.el= which is Emacs' builtin
  package manager.

  *NOTE*: We use =(require lib)= when the filename is =lib.el= which can be any file
  saved on your system. Make sure to set the =load-path= as given in the previous
  sections for =require= to work properly.
  ...
  #+end_src
* Install =use-package=
  =use-package= is a convenience function for package definitions in Emacs Lisp.
  Since Emacs 29 =use-package= is distributed as a builtin Emacs function.
  #+begin_src emacs-lisp :tangle init.el
    (when (< emacs-major-version 29)
...
* Install =f= and =s=
  ...
  *NOTE*: =:ensure= is a =use-package= specific term which when =t= (true) means that
  the corresponding package will be fetched using =package.el= if not already
...
** Enable Ctrl+click
   Emacs supports =Ctrl+click= to jump to a function starting from version 31. The
   functionality already existed on =M-.= though.
   #+begin_src emacs-lisp :tangle init.el
...
** Dired - File manager
   ...
   - [[https://github.com/alexluigit/dirvish][dirvish]] - [[https://github.com/ranger/ranger][ranger]] alternative
   #+begin_src emacs-lisp :tangle init.el
     (use-package dired
...
** One tab per project
   ...
   #+end_src
** Casual shortcuts
   Casual brings in transient like menu to lot of Emacs functionality.
   #+begin_src emacs-lisp :tangle init.el
