  ...
  #+begin_src emacs-lisp :tangle (early-init)
    (use-package server
      ...
      (when (not (server-running-p))
        ...
        (server-start)))
  #+end_src
  A daemon has no initial frame.  Create a graphical frame after startup when
  an X11 or Wayland display is available.  Emacs started normally in a GUI or
  TTY already has an initial frame, while a headless daemon needs
  =emacsclient -t= to attach a terminal.
  #+begin_src emacs-lisp :tangle (early-init)
